...
def main():
    ...
    api_key = (
        b64decode(options.api_key.encode()) if (options.api_key is not None) else None
    )
...
def parse_args():
    ...
    parser.add_option(
        ...
        action='store_true',
        dest='verbose',
        help="Print request URL and full responst to stderr.",
    )
    parser.add_option(
        '-n',
        '--noexec',
        action='store_true',
        dest='noexec',
        help="Don't send the request, just print the URL.",
    )
    parser.add_option(
        '-V',
        '--version',
        ...
        choices=['1.0', '1.1', '2.0'],
        default='2.0',
        help="The API version to use (1.0, 1.1, 2.0). [%default]",
    )
    parser.add_option(
        '-u',
        '--base-url',
        dest='base_url',
        help="Base URL of the api. Defaults to a version-appropriate URL on api.yubico.com.",
    )
    parser.add_option(
        '-s',
        '--ssl',
        action='store_true',
        dest='ssl',
        help="Use an https url by default.",
    )
    parser.add_option(
        '-i', '--api-id', dest='api_id', default='1', help="Your API ID. [%default]"
    )
    parser.add_option(
        '-k', '--api-key', dest='api_key', help="Your base64-encoded API key."
    )
    parser.add_option(
        '-t',
        '--timestamp',
        action='store_true',
        dest='timestamp',
        help="(Version 1.1+) include timestamp and counter information in response.",
    )
    parser.add_option('--sl', dest='sl', help="(Version 2.0+) Request server syncing.")
    parser.add_option(
        '--timeout',
        dest='timeout',
        help="(Version 2.0+) Seconds to wait for sync response.",
    )
